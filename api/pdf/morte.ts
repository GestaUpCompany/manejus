...
interface DiagnosticoItem { chave: string; valor: string; observacao: string }
interface LinhaMorte {
  id: string; data: string; data_hora: string; lote_id: string | null; lote_nome: string | null
  pasto: string | null; sexo: string | null; raca: string | null; idade: string | null
  peso_vivo: number | null; causa_morte: string | null; categoria: string | null
  categoria_outros: string | null; brinco: string | null; chip: string | null; escore: number | null
  nutricao_atual: string | null; nutricao_anterior: string | null
  diagnosticos: Record<string, DiagnosticoItem> | null; observacao_identificacao: string | null; nome_usuario: string | null
}
interface AgregadoItem { label: string; valor: number }
interface ResumoMorte {
  total_mortes: number; media_por_dia: number | null; peso_medio: number | null
  causa_mais_frequente: string | null; causa_mais_frequente_count: number | null
  por_causa: AgregadoItem[]; por_categoria: AgregadoItem[]; por_sexo: AgregadoItem[]
  frequencia_diagnosticos: AgregadoItem[]; taxa_mortalidade?: number | null; rebanho_total?: number
  perda_estimada?: number | null; peso_total_perdido?: number | null; insights?: string
  periodo_anterior?: { total_mortes: number; taxa_mortalidade: number | null; data_inicio: string; data_fim: string } | null
}
interface PDFData {
  dataInicio: string; dataFim: string; fazendaNome: string; logoGestao: string; logoFazenda: string
  resumo: ResumoMorte; linhas: LinhaMorte[]; chartTempo: string | null; chartCausa: string | null
  chartCategoria: string | null; chartSexo: string | null
}
...
function renderMorteHtml(data: PDFData): string {
  const { resumo } = data
