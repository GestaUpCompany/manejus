-- Complemento da fase 2. A migration 20260916160000 foi registrada antes da
-- aplicação completa; este arquivo contém a implementação executável.
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(COALESCE(NEW.itens,'[]'::jsonb)) LOOP
    ...
    ELSIF v_tipo='devolucao' THEN
      ...
      IF v_ret_id IS NOT NULL AND v_ret_idx IS NOT NULL THEN
        SELECT x.value INTO v_orig FROM public.registros_almoxarifado r,LATERAL jsonb_array_elements(COALESCE(r.itens,'[]'::jsonb)) WITH ORDINALITY x(value,idx) WHERE r.id=v_ret_id AND r.fazenda_id=NEW.fazenda_id AND COALESCE(r.tipo,'retirada')='retirada' AND x.idx-1=v_ret_idx AND (x.value->>'itemId')::uuid=v_item_id;
      ...
      ELSE
        SELECT GREATEST(0,COALESCE(SUM(REPLACE(COALESCE(i.value->>'quantidade','0'),',','.')::numeric),0)-COALESCE((SELECT SUM(m.quantidade_aprovada) FROM public.movimentacoes_almoxarifado m JOIN public.registros_almoxarifado d ON d.id=m.registro_origem_id WHERE m.fazenda_id=NEW.fazenda_id AND m.item_id=v_item_id AND m.tipo_movimentacao='devolucao' AND m.retirada_id IS NULL AND m.deleted_at IS NULL AND d.quem_pegou=NEW.quem_pegou),0)) INTO v_pending FROM public.registros_almoxarifado r,LATERAL jsonb_array_elements(COALESCE(r.itens,'[]'::jsonb)) i WHERE r.fazenda_id=NEW.fazenda_id AND COALESCE(r.tipo,'retirada')='retirada' AND r.quem_pegou=NEW.quem_pegou AND r.deleted_at IS NULL AND (i.value->>'itemId')::uuid=v_item_id AND i.value->>'necessitaDevolucao'='S';
      END IF;
...
BEGIN RETURN QUERY SELECT (x.value->>'itemId')::uuid,ia.nome,ia.unidade,r.id,(x.idx-1)::integer,GREATEST(0,REPLACE(COALESCE(x.value->>'quantidade','0'),',','.')::numeric-COALESCE((SELECT SUM(m.quantidade_aprovada) FROM public.movimentacoes_almoxarifado m WHERE m.retirada_id=r.id AND m.retirada_item_index=x.idx-1 AND m.tipo_movimentacao='devolucao' AND m.deleted_at IS NULL),0)),x.value->>'prazoDevolucao',r.quem_pegou FROM public.registros_almoxarifado r CROSS JOIN LATERAL jsonb_array_elements(COALESCE(r.itens,'[]'::jsonb)) WITH ORDINALITY x(value,idx) JOIN public.itens_almoxarifado ia ON ia.id=(x.value->>'itemId')::uuid WHERE r.fazenda_id=p_fazenda_id AND COALESCE(r.tipo,'retirada')='retirada' AND r.deleted_at IS NULL AND x.value->>'necessitaDevolucao'='S' AND (p_quem_pegou IS NULL OR r.quem_pegou=p_quem_pegou) AND GREATEST(0,REPLACE(COALESCE(x.value->>'quantidade','0'),',','.')::numeric-COALESCE((SELECT SUM(m.quantidade_aprovada) FROM public.movimentacoes_almoxarifado m WHERE m.retirada_id=r.id AND m.retirada_item_index=x.idx-1 AND m.tipo_movimentacao='devolucao' AND m.deleted_at IS NULL),0))>0 ORDER BY r.data,ia.nome; END; $$ LANGUAGE plpgsql SECURITY DEFINER SET search_path=public;
