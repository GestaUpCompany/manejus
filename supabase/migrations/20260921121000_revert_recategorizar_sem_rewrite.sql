-- Reverte recategorizar_lote_categoria para a versão anterior à migration
-- 20260921120000 (sem reescrita de registros). A reescrita mostrou-se
-- estruturalmente incorreta: registros com lote_destino_id são compartilhados
-- entre dois lotes e renomear categoria corrompia a contagem do outro lado.
-- A versão restaurada mantém o comportamento original (rename in-place apenas
-- em lote_categorias). A solução definitiva via quant_base fica pendente em
-- 20260921130000_recategorizar_quant_base.sql, aguardando revisão.
