-- Restaura o cálculo de rebanho_total por lote_categorias.quant_atual na RPC de
-- mortalidade. A migration 20260911161139 já tinha essa correção, mas as
-- recriações posteriores da função (a partir de 20260916120000) regrediram o
-- denominador para lotes.n_cabecas/numero_cabecas, que é NULL em fazendas como
-- a Brilhante, zerando o rebanho e anulando a taxa de mortalidade.

DROP FUNCTION IF EXISTS public.get_dados_relatorio_morte(uuid, date, date, numeric);
DROP FUNCTION IF EXISTS public.get_dados_relatorio_morte(uuid, date, date);
...
DECLARE
  ...
  v_total_mortes_geral bigint;
  v_taxa_mortalidade_geral numeric;
...
BEGIN
  ...
  -- Totais históricos (sem filtro de data): base da taxa de mortalidade
  -- acumulada exibida na primeira página do relatório.
  SELECT COUNT(*)
  INTO v_total_mortes_geral
  FROM registros_morte r
  WHERE r.fazenda_id = v_fazenda_id
    AND r.deleted_at IS NULL;

  v_taxa_mortalidade_geral := CASE WHEN v_rebanho_total > 0 THEN round((v_total_mortes_geral::numeric / v_rebanho_total * 100)::numeric, 2) ELSE NULL END;
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'nome', p.nome,
    'geometry', extensions.ST_AsGeoJSON(p.geometria)::jsonb
  ...
  RETURN jsonb_build_object(
    ...
    'dados', jsonb_build_object(
      ...
      'total_mortes_geral', v_total_mortes_geral,
      'taxa_mortalidade_geral', v_taxa_mortalidade_geral,
