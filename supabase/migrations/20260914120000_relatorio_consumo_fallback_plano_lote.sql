-- RPC get_dados_relatorio_consumo: fallback para plano nutricional ativo do lote
-- ----------------------------------------------------------------------------
-- Problema: a RPC derivava dieta, dias e data_prevista_final apenas de:
--   - lote_categorias.formulacao_id
--   - planos_nutricionais com lote_categoria_id = lc.id (plano por categoria)
--
-- O modelo atual é plano por lote (planos_nutricionais.lote_id, lote_categoria_id NULL).
-- Lotes com plano ativo no nível do lote ficavam com pills vazios no relatório público,
-- especialmente quando lote_categorias.formulacao_id era sobrescrito para NULL por edições
-- do formulário de lote.
--
-- Correção: a CTE cats_por_lote passa a trazer também o plano ativo do lote
-- (pn_lote) e a personalização (plano_categoria_personalizacao). As colunas
-- finais usam COALESCE para preferir plano da categoria, cair para plano do lote,
-- e finalmente para lotes.formulacao_id. data_prevista_final ganha fallback
-- data_inicio + periodo_dias quando data_meta_projetada está ausente.
