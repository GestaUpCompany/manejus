...
export function EstoqueAlmoxarifado() {
  ...
  const carregar = useCallback(async () => {
    ...
    const { data: revisaoData, count } = await supabase.from('movimentacoes_almoxarifado').select('id,item_id,quantidade,quantidade_aprovada,data,observacao,itens_almoxarifado(nome,unidade)', { count: 'exact' }).eq('fazenda_id', fazendaId).eq('requer_revisao', true).is('deleted_at', null).order('data', { ascending: false })
  ...
  const aprovarDevolucao = async (movimento: any) => {
    const { error } = await supabase.from('movimentacoes_almoxarifado').update({ quantidade_aprovada: movimento.quantidade, requer_revisao: false }).eq('id', movimento.id).eq('fazenda_id', fazendaId!)
    if (error) setErro(error.message)
    else carregar()
  }

  const abrirHistorico = async (item: Item) => {
    const { data } = await supabase.from('movimentacoes_almoxarifado').select('id,tipo_movimentacao,quantidade,custo_unitario,valor_total,data,origem,fornecedor,observacao').eq('fazenda_id', fazendaId!).eq('item_id', item.id).is('deleted_at', null).order('data', { ascending: false }).order('created_at', { ascending: false })
