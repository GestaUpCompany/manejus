  ...
  return (
    <div className="space-y-4 sm:space-y-6 min-w-0">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <h2 className="text-xl sm:text-2xl font-bold text-content-strong">Caderneta de Abastecimento</h2>
      </div>
      ...
      <Card className="bg-surface-1 p-4 sm:p-6" disableHover>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-3">
          ...
          <Button
            onClick={() => exportToXLSX(filteredRegistros, ABASTECIMENTO_EXPORT_CONFIG, fazendaNome)}
      ...
      {registros.length === 0 ? (
        <Card className="bg-surface-1 p-4 sm:p-6 text-center" disableHover>
          <p className="text-content-muted">Nenhum registro de abastecimento encontrado</p>
        </Card>
            ...
            {filteredRegistros.map((registro) => (
              ...
              >
                ...
                <div className="space-y-2 text-xs sm:text-sm">
                  ...
                  <div className="flex justify-between">
                    <span className="text-content-muted">Quem Abasteceu:</span>
                    <span className="text-content-strong font-medium">{registro.quem_abasteceu || '-'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-content-muted">Operador/Motorista:</span>
                    <span className="text-content-strong font-medium">{registro.operador_motorista || '-'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-content-muted">Máquina/Veículo:</span>
                    <span className="text-content-strong font-medium">{registro.maquina_veiculo || '-'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-content-muted">Placa:</span>
                    <span className="text-content-strong font-medium">{registro.placa || '-'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-content-muted">Total Abastecido:</span>
                    <span className="text-content-strong font-medium">{registro.total_abastecido ? `${registro.total_abastecido} L` : '-'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-content-muted">Combustível:</span>
                    <span className="text-content-strong font-medium">{registro.combustivel || '-'}</span>
                  </div>
                </div>
              </Card>
            ))}
            ...
            <table className="min-w-full divide-y divide-border-base">
              <thead className="bg-surface-2">
                <tr>
                  ...
                  <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-content-muted uppercase tracking-wider">Quem Abasteceu</th>
                  <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-content-muted uppercase tracking-wider">Operador/Motorista</th>
                  <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-content-muted uppercase tracking-wider">Máquina/Veículo</th>
                  <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-content-muted uppercase tracking-wider">Placa</th>
                  <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-content-muted uppercase tracking-wider">Total Abastecido</th>
                  <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-content-muted uppercase tracking-wider">Combustível</th>
                </tr>
              </thead>
