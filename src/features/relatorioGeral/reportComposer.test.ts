...
describe('compositor do relatório geral', () => {
  it('cria capa e uma página vazia com paginação global', async () => {
    ...
    expect(html).toContain('.cover-organizations{display:flex;align-items:center;gap:14px;padding:4mm 6mm')
    expect(html).toContain('.cover-logo-box{height:24mm')
    expect(html).toContain('.final-logos{display:flex;align-items:center;gap:14px;margin-top:10mm;padding:4mm 6mm')
  })
  ...
  it('compõe renderizadores diferentes e mantém a ordem informada', async () => {
    ...
    expect(html).toContain('Página 2 de 9')
    expect(html).toContain('Página 9 de 9')
  })
})
