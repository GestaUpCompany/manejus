...
- Backfill é operação de dados (fora da migration): `UPDATE ... FROM currais` casando `lower(trim(nome)) = lower(trim(pasto_curral))` por fazenda. Fazenda de testes: 7/7 linhas preenchidas (100%), incluindo excluídas. Jacamim (única fazenda em produção com confinamento) fica para depois da validação na teste — até lá suas linhas têm `curral_id` NULL e não entram no índice.
...
**Disparador**: quando mencionar `curral_id` em leitura, unicidade de leitura de cocho, duplicata de leitura no sync, ou backfill de `curral_id` (Jacamim), ler esta seção.
